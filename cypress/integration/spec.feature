Feature: test task for Exberry

    Background:
        Given a calendar
            | Name       | TimeZone | MarketOpen | MarketClose | TradingDays                                              | Holidays |
            | <calendar> | +01:00   |            |             | MONDAY,TUESDAY,WEDNESDAY,THURSDAY,FRIDAY,SATURDAY,SUNDAY |          |
        And an mp with an api key with full permissions
        And an instrument
            | symbol   | Description | Calendar   | Status | QuoteCurrency | PricePrecision | QuantityPrecision | MinQuantity | MaxQuantity |
